...
module tri_prisim(length, width, height){
    a = 2*height/sqrt(2);
    scale([1,width/(2*height),1]) xrot(-45-90)
        wedge([length,a,a], anchor="hypot");
}
...
paddingFront_calc        = generalPadding + 17;
...
cameraPodiumWidth = cableCutoutWidth+5;
cameraPodiumHeight = 19;
cableCutoutHeight = cameraPodiumHeight-4;
cameraPodiumLength = 10;

cameraPodiumCenter_relpcb = [
    espLength+paddingFront_calc/3, 
    espWidth/2,
    0
];
cameraPodiumCenter = [
...
// (n) = { yappCenter }
cutoutsFront =  [
                    [0.95*espWidth, 0, 4, 0, 0, yappCircle, yappCenter],
                ];
